...
// (lascia qui dentro i dati delle bevande IDENTICI a come li hai già)
...
// ===== ABOUT / ALLERGENS / HERO / CONTACT TEXT =====
const aboutData = { /* invariato, come già ce l'hai */ };
const allergensData = { /* invariato */ };
const heroData = { /* invariato */ };
const contactData = { /* invariato */ };
...
function renderAllergens(lang = "es") {
  const titleEl = document.getElementById("allergens-title");
  const textEl  = document.getElementById("allergens-text");
  if (!titleEl || !textEl) return;
  titleEl.textContent = allergensData[lang].title;
  textEl.textContent  = allergensData[lang].text;
}
